...
//    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    ...
    if ([self dzn_shouldDisplay] || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (!view.superview) {
            ...
            if (([self isKindOfClass:[UITableView class]] || [self isKindOfClass:[UICollectionView class]]) && self.subviews.count > 1) {
                [self insertSubview:view atIndex:10000];
...
//    [self addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(selectPickUpTypeAction)]];
...
- (UIView *)contentView
{
    if (!_contentView)
    {
        ...
        _contentView.backgroundColor = AppBgBlueGrayColor;
