...
//    审核（同意/拒绝/发货） approve/refuse/deliver
    ...
    self.btn4 = [UIButton buttonWithTitie:@"继续添加" WithtextColor:COLOR(@"#FFFFFF") WithBackColor:AppTitleBlueColor WithBackImage:nil WithImage:nil WithFont:17 EventBlock:^(id  _Nonnull params) {
        ...
        sellGoodsScanVC.orderID = self.dataModel.orderID;
            ...
            if ([operation.urlTag isEqualToString:Path_stock_orderDetail]) {//进货单详情
                ...
                if ([parserObject.code isEqualToString:@"200"]) {
                    ...
                    if(self.controllerType == PurchaseOrderManageVCTypeDeliveryApply){
                        [self handleTabDefInfoData:[NSString stringWithFormat:@"快递单号：%@",self.dataModel.expressCode]];
                        [self handleTabMarkData:NO];
                    }
//
                    [self.tableView reloadData];
...
//                    self.dataModel = [XwOrderDetailModel mj_objectWithKeyValues:parserObject.datas[@"datas"]];
                ...
                if ([parserObject.code isEqualToString:@"200"]) {
                    ...
                    self.orderRemarks = self.dataModel.remarks;
...
//    NSString *dateString=[dateFormtter stringFromDate:inputDate];
    ...
    if(![self.dataModel.wishReceiveDate isEqualToString:@""]){
        self.wishReceiveDate = self.dataModel.wishReceiveDate;
    }
...
//AD审核信息/快递单号
-(void)handleTabDefInfoData:(NSString*)defInfo{
    
    CGSize size = [defInfo sizeWithFont:FONT(14) Size:CGSizeMake((SCREEN_WIDTH -30 -16), 2000)];
    CGFloat height = 40;
    if(size.height  > 40){
        height  = size.height + 40;
    }
...
- (void)httpPath_dallot_transferOperate:(NSString*)operate
{
    ...
    if(array.count == 0 && [operate isEqualToString:@"deliver"]){
